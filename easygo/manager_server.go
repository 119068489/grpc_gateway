...
type IServerManager interface {
	AddServerInfo(srv *ServerInfo)
	DelServerInfo(id server_id)
	GetServerInfo(serverId server_id) *ServerInfo
	GetIdelServer(t int) *ServerInfo
	GetAllServers(t int) []*ServerInfo
}
...
func (serverSelf *ServerInfoManager) Init(me IServerManager) {
	serverSelf.Me = me
...
func (serverSelf *ServerInfoManager) ChangeServerState(serverId server_id, st int) {
	serverSelf.Mutex.Lock()
	defer serverSelf.Mutex.Unlock()
...
//负载均衡，分配一台服务器
func (serverSelf *ServerInfoManager) GetIdelServer(t int) *ServerInfo {
	serverSelf.Mutex.Lock()
	defer serverSelf.Mutex.Unlock()
	temMap := make(map[int]int, len(serverSelf.ServerInfo))
...
func (serverSelf *ServerInfoManager) GetAllServers(t int) []*ServerInfo {
	serverSelf.Mutex.Lock()
	defer serverSelf.Mutex.Unlock()
...
// 返回人数最少的服务器ID
func (serverSelf *ServerInfoManager) SortMapByValue(m map[int]int) int {
