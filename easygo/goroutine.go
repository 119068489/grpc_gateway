...
func (grSelf *Goroutine) Init(me IGoroutine) {
	grSelf.Me = me
	grSelf.CompleteEvent = make(chan bool)
	grSelf.Value = None
}

func (grSelf *Goroutine) Start(function interface{}, args ...interface{}) {
	grSelf.mutex.Lock()
	defer grSelf.mutex.Unlock()
	if grSelf.started {
		return
	}
	grSelf.started = true
	...
	go func() {
		...
		defer func() {
			...
			if r != nil {
				...
				value = r
				grSelf.Me.OnPanic(callStack, r)
			}
			grSelf.mutex.Lock()
			defer grSelf.mutex.Unlock()
			grSelf.Value = value
			close(grSelf.CompleteEvent)
		}()
...
func (grSelf *Goroutine) OnPanic(callStack string, recoverVal interface{}) {

}

func (grSelf *Goroutine) Join(timeout ...time.Duration) {
	grSelf.Me.Get(true, timeout...)
}

// 如果超时 ，则返回 TimeoutError。有异常则向上波及。否则返回正常执行的结果
func (grSelf *Goroutine) Get(block bool, timeout ...time.Duration) interface{} {
	...
	if !block {
		return timeout //NewTimeoutError("timeout") // todo 把具体时间放进 error 对象里面
	}
	...
	select {
	case <-grSelf.CompleteEvent:
		if timer != nil {
			timer.Stop()
		}
		grSelf.mutex.Lock() // 取 grSelf.Value 要上锁
		defer grSelf.mutex.Unlock()
		return grSelf.Value
...
// func (grSelf *Goroutine) Successful() bool {
// 	if grSelf.Value == None {
// 		return false
// 	}else if _, ok := grSelf.Value.(error); ok { //
// 		return false
// 	}
// 	return true
// }

func (grSelf *Goroutine) GetCompleteEvent() chan bool {
	return grSelf.CompleteEvent
}

func (grSelf *Goroutine) GetValue() interface{} {
	grSelf.mutex.Lock()
	defer grSelf.mutex.Unlock()
	return grSelf.Value
}
